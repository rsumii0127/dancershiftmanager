<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-6">
        ...
        @endif
        <form method="POST" action="{{ route('present.history.update') }}">
            @csrf
            @if(session('message'))
            <div class="text-red-600 font-bold">
                {{session('message')}}
            ...
            <div class="mt-8">
                ...
                <div class="w-full flex flex-col">
                    ...
                    <input type="text" name="brand" class="w-auto py-2 border border-gray-300 rounded-md" id="brand" value={{ old('present_brand')}}>
                </div>
                <div class="w-full flex flex-col">
                    <label for="price" class="font-semibold mt-4">価格</label>
                    <x-input-error :messages="$errors->get('price')" class="mt-2" />
                    <input type="text" name="price" class="w-auto py-2 border border-gray-300 rounded-md" id="price" value={{ old('present_price')}}>
                </div>
                <div class="w-full flex flex-col">
